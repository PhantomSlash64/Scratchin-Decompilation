...
namespace UnityEngine.U2D.Animation.TriangleNet.Voronoi
{
	internal abstract class VoronoiBase : DcelMesh
	{
		...
		protected void Generate(Mesh mesh)
		{
			...
			UnityEngine.U2D.Animation.TriangleNet.Topology.DCEL.Vertex[] array = new UnityEngine.U2D.Animation.TriangleNet.Topology.DCEL.Vertex[mesh.triangles.Count + mesh.hullsize];
			...
			ConnectEdges(map);
			vertices = new List<UnityEngine.U2D.Animation.TriangleNet.Topology.DCEL.Vertex>(array);
			faces = new List<Face>(array2);
		}

		protected List<HalfEdge>[] ComputeVertices(Mesh mesh, UnityEngine.U2D.Animation.TriangleNet.Topology.DCEL.Vertex[] vertices)
		{
			...
			foreach (Triangle triangle in mesh.triangles)
			{
				...
				UnityEngine.U2D.Animation.TriangleNet.Topology.DCEL.Vertex vertex = factory.CreateVertex(point.x, point.y);
				vertex.id = id;
				vertices[id] = vertex;
		...
		protected void ComputeEdges(Mesh mesh, UnityEngine.U2D.Animation.TriangleNet.Topology.DCEL.Vertex[] vertices, Face[] faces, List<HalfEdge>[] map)
		{
			...
			foreach (Triangle triangle in mesh.triangles)
			{
				...
				for (int i = 0; i < 3; i++)
				{
					...
					if (id < id2 || id2 < 0)
					{
						UnityEngine.U2D.Animation.TriangleNet.Geometry.Vertex vertex = otri.Org();
						UnityEngine.U2D.Animation.TriangleNet.Geometry.Vertex vertex2 = otri.Dest();
						Face face = faces[vertex.id];
						Face face2 = faces[vertex2.id];
						UnityEngine.U2D.Animation.TriangleNet.Topology.DCEL.Vertex vertex3 = vertices[id];
						UnityEngine.U2D.Animation.TriangleNet.Topology.DCEL.Vertex vertex4;
						HalfEdge halfEdge;
						HalfEdge halfEdge2;
						if (id2 < 0)
						{
