	...
	internal static class Converter
	{
		...
		private static List<Otri>[] SetNeighbors(Mesh mesh, ITriangle[] triangles)
		{
			...
			foreach (Triangle triangle in mesh.triangles)
			{
				...
				while (item.orient < 3)
				{
					...
					if (otri.tri.id != -1)
					{
						UnityEngine.U2D.Animation.TriangleNet.Geometry.Vertex vertex = item.Dest();
						UnityEngine.U2D.Animation.TriangleNet.Geometry.Vertex vertex2 = item.Apex();
						do
						{
							UnityEngine.U2D.Animation.TriangleNet.Geometry.Vertex vertex3 = otri.Dest();
							UnityEngine.U2D.Animation.TriangleNet.Geometry.Vertex vertex4 = otri.Apex();
							if (vertex2 == vertex3)
							{
								item.Lprev(ref ot);
								ot.Bond(ref otri);
		...
		private static void SetSegments(Mesh mesh, Polygon polygon, List<Otri>[] vertexarray)
		{
			...
			if (mesh.behavior.Poly)
			{
				...
				{
					os.seg = value;
					UnityEngine.U2D.Animation.TriangleNet.Geometry.Vertex vertex = polygon.Segments[num3].GetVertex(0);
					UnityEngine.U2D.Animation.TriangleNet.Geometry.Vertex vertex2 = polygon.Segments[num3].GetVertex(1);
					...
					while (os.orient < 2)
					{
						...
						UnityEngine.U2D.Animation.TriangleNet.Geometry.Vertex vertex3 = os.Org();
						bool flag = true;
						while (flag && otri.tri.id != -1)
						{
							UnityEngine.U2D.Animation.TriangleNet.Geometry.Vertex vertex4 = otri.Dest();
							if (vertex3 == vertex4)
							{
		...
		internal static DcelMesh ToDCEL(Mesh mesh)
		{
			...
			UnityEngine.U2D.Animation.TriangleNet.Topology.DCEL.Vertex[] array = new UnityEngine.U2D.Animation.TriangleNet.Topology.DCEL.Vertex[mesh.vertices.Count];
			...
			{
				UnityEngine.U2D.Animation.TriangleNet.Topology.DCEL.Vertex vertex = new UnityEngine.U2D.Animation.TriangleNet.Topology.DCEL.Vertex(value.x, value.y);
				vertex.id = value.id;
			...
			foreach (Triangle triangle2 in mesh.triangles)
			{
				...
				for (int i = 0; i < 3; i++)
				{
					...
					{
						Face face = array2[id];
						UnityEngine.U2D.Animation.TriangleNet.Geometry.Vertex vertex2 = otri.Org();
						UnityEngine.U2D.Animation.TriangleNet.Geometry.Vertex vertex3 = otri.Dest();
