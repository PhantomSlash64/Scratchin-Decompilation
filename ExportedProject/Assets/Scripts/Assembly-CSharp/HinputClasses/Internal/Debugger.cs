	...
	public class Debugger : MonoBehaviour
	{
		...
		private void TestInfo()
		{
			...
			if (gamepadListsOnLPressed && Input.GetKeyDown(KeyCode.L))
			{
				...
				Debug.Log("current gamepad buttons : " + ToString(gamepad2.buttons.Select((Pressable button) => button.name).ToList()) + ", current gamepad sticks : " + ToString(gamepad2.sticks.Select((Stick stick) => stick.name).ToList()));
			}
