...
namespace HinputClasses.Internal
{
	public class Vibration
	{
		...
		private PresetCurves GetCurves(VibrationPreset vibrationPreset, float left, float right, float duration)
		{
			...
			animationCurve.keys = (from key in animationCurve.keys.ToList()
				select new Keyframe(key.time * duration, key.value * left)).ToArray();
			animationCurve2.keys = (from key in animationCurve2.keys.ToList()
				select new Keyframe(key.time * duration, key.value * right)).ToArray();
