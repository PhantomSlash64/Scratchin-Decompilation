...
public class MusicBox : MonoBehaviour
{
	...
	private void Update()
	{
		if (active && !busy)
		{
			...
			else if (player.GetAxisRaw("Vertical") < 0f)
			{
				Down.SetTrigger("go");
				Downs.Play();
				busy = true;
		...
		if (Close.activeInHierarchy && player.GetButtonDown("Butt1"))
		{
			ItemGet.SetActive(value: false);
			player1.SetTrigger("CloseNotif");
			music.mute = false;
		}
		if (player.GetAxisRaw("Vertical") == 0f && player.GetAxisRaw("Horizontal") == 0f)
		{
			busy = false;
		}
	}
	...
	private void On_End(KoreographyEvent evt)
	{
		...
		if (points >= pointsNeeded)
		{
			...
			good.Play();
			player1.SetTrigger("ItemGet");
			Lock.SetActive(value: false);
			ES3.Save(Sticker, 1f);
			ES3.Save("hasStickers", 1f);
		}
		else
		{
			bad.Play();
	...
	{
		if (active && player.GetAxisRaw("Vertical") > 0f && !busy)
		{
			points++;
		}
	}
	...
	{
		if (active && player.GetAxisRaw("Horizontal") < 0f && !busy)
		{
			points++;
		}
	}
