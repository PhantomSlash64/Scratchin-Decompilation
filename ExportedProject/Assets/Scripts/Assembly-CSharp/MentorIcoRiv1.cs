...
public class MentorIcoRiv1 : MonoBehaviour
{
	...
	private void Start()
	{
		...
		firstTime = ES3.Load("FirstTimeNWAV", defaultValue);
		if (firstTime == 0f)
		{
			FirstTime = true;
		}
	...
	private void Bonus()
	{
		if (PlayerIcoRiv1.HiFi && !BonusAdded)
		{
			BonusAdded = true;
			HiFiBonus.SetActive(value: true);
			ResultsSlider.value += 25f;
		}
		else if (PlayerIcoRiv1.WoahFi && !BonusAdded)
		{
			BonusAdded = true;
	...
	private void On_Intro(KoreographyEvent evt)
	{
		if (IntroCount == 0)
		{
			IntroVid.SetActive(value: false);
			IntroHand.SetActive(value: true);
			IntroCount++;
		}
		else
		{
			IntroHand.SetActive(value: false);
	...
	private void On_Abutt(KoreographyEvent evt)
	{
		...
		if (Butt1Presses == 0f)
		{
			...
			if (Line == 4f)
			{
				...
				Scene1.SetActive(value: true);
				Scene1cam.SetActive(value: false);
				Scene1cam2.SetActive(value: true);
	...
	private void On_LineStart(KoreographyEvent evt)
	{
		...
		if (Line == 7f)
		{
			...
			{
				Line7Peasy.SetActive(value: true);
			}
		}
		if (Line == 1f)
		{
	...
	private void Update()
	{
		...
		if (ResultsTime)
		{
			...
			if (ACPdone)
			{
				if (Difficulty == "Easy")
				{
					highScore = ES3.Load("HighScoreCCIeasy", defaultValue);
				}
				else if (Difficulty == "Normal")
				{
				...
				if ((float)(PlayerIcoRiv1.FunkStatic + PlayerIcoRiv1.StyleStatic + PlayerIcoRiv1.ACPStatic) > highScore)
				{
					NewRecord = true;
					if (Difficulty == "Easy")
					{
						ES3.Save("HighScoreCCIeasy", (float)(PlayerIcoRiv1.FunkStatic + PlayerIcoRiv1.StyleStatic + PlayerIcoRiv1.ACPStatic));
					}
					else if (Difficulty == "Normal")
					{
						ES3.Save("HighScoreCCI", (float)(PlayerIcoRiv1.FunkStatic + PlayerIcoRiv1.StyleStatic + PlayerIcoRiv1.ACPStatic));
					}
				}
			}
		}
	...
	private void On_EndSong(KoreographyEvent evt)
	{
		...
		if (FirstTime)
		{
			ES3.Save("FirstTimeCCI", 1f);
		}
		if (SaveFile == 1 && InStory)
		{
			ES3.Save("LastSongStory", 1);
		}
	}
}
