...
namespace UnityEngine.U2D.Animation.TriangleNet.Voronoi.Legacy
{
	...
	internal class SimpleVoronoi : IVoronoi
	{
		...
		private void ComputeCircumCenters()
		{
			...
			bounds.Resize(num / 10.0, num / 10.0);
		}
		...
		private void ConstructCell(VoronoiRegion region)
		{
			...
			if (!rayPoints.TryGetValue(hash, out var value))
			{
				...
				value = IntersectionHelper.BoxRayIntersection(bounds, points[ot.tri.id], vertex.y - vertex2.y, vertex2.x - vertex.x);
			...
			if (!rayPoints.TryGetValue(hash, out value))
			{
				...
				value = IntersectionHelper.BoxRayIntersection(bounds, points[ot.tri.id], vertex3.y - vertex.y, vertex.x - vertex3.x);
				value.id = count + rayIndex;
				rayPoints.Add(hash, value);
				points[count + rayIndex] = value;
				rayIndex++;
		...
		private IEnumerable<IEdge> EnumerateEdges()
		{
			...
			{
				IEnumerator<Point> enumerator2 = region.Vertices.GetEnumerator();
				enumerator2.MoveNext();
				Point current2;
				Point point = (current2 = enumerator2.Current);
				while (enumerator2.MoveNext())
				{
					if (region.ID < region.GetNeighbor(current2).ID)
					{
						list.Add(new Edge(current2.id, enumerator2.Current.id));
					}
					current2 = enumerator2.Current;
				}
				if (region.Bounded && region.ID < region.GetNeighbor(current2).ID)
				{
					list.Add(new Edge(current2.id, point.id));
				}
			}
			return list;
		}
