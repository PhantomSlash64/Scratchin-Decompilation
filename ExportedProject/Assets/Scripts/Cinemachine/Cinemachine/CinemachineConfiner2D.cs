...
namespace Cinemachine
{
	...
	[HelpURL("https://docs.unity3d.com/Packages/com.unity.cinemachine@2.7/manual/CinemachineConfiner2D.html")]
	public class CinemachineConfiner2D : CinemachineExtension
	{
		...
		protected override void PostPipelineStageCallback(CinemachineVirtualCameraBase vcam, CinemachineCore.Stage stage, ref CameraState state, float deltaTime)
		{
			...
			vector = bakedSolution.ConfinePoint(in pointToConfine);
		...
		{
			float f = ((!state.Lens.Orthographic) ? (cameraPosLocalZ * Mathf.Tan(state.Lens.FieldOfView * 0.5f * ((float)Math.PI / 180f))) : state.Lens.OrthographicSize);
			return Mathf.Abs(f);
		}
